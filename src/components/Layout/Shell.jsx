...
export default function Shell({ user, route, setRoute, onLogout, children }) {
  ...
  // Cấu hình Menu
  const MENU_ITEMS = [
    { id: 'dashboard', label: 'Tổng quan', icon: LayoutDashboard, roles: ['MANAGER', 'CASHIER', 'STAFF'] },
    { id: 'order',     label: 'Sơ đồ bàn', icon: UtensilsCrossed, roles: ['MANAGER', 'CASHIER', 'STAFF'] },
    { id: 'menu',      label: 'Thực đơn',  icon: Coffee,          roles: ['MANAGER', 'CASHIER', 'STAFF'] },
    { id: 'staff',     label: 'Nhân viên', icon: Users,           roles: ['MANAGER'] }, // Chỉ Manager
    { id: 'reports',   label: 'Báo cáo',   icon: BarChart3,       roles: ['MANAGER'] }, // Chỉ Manager
  ]

  // Lọc menu theo quyền
  const allowedMenu = MENU_ITEMS.filter(item => item.roles.includes(user?.role || 'STAFF'))
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0 z-20 h-full">
        ...
        {/* 2. MENU LIST (Cuộn riêng phần này nếu danh sách dài) */}
        ...
        {/* 3. USER & LOGOUT (Cố định dưới cùng - Quan trọng) */}
      ...
      <main className="flex-1 overflow-y-auto relative h-full">
        {/* Header mobile (nếu cần) hoặc padding cho nội dung */}
