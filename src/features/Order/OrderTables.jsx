...
// Helper: Sort tables (e.g. "Bàn 1", "Bàn 10", "Bàn 2")
...
export default function OrderTables({ user, setRoute, setActiveTable, setActiveOrderId }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      setTables(list.sort(sortTables))
      setLoading(false)
    })
    return () => unsub()
  }, [])
  ...
  const processBulkSetup = async () => {
    ...
    try {
        ...
        setActiveZone("Tầng 1")
    } catch (error) { 
        console.error(error)
        setSetupError("Lỗi khi lưu dữ liệu.") 
    } finally { 
  ...
  const handleQuickPay = (e, table) => { 
      e.stopPropagation()
      setPayingTable(table)
      setPayingOrderId(table.currentOrderId) 
  }
  ...
  return (
    <div className="h-full flex flex-col animate-fadeIn bg-slate-50">
      ...
      {payingTable && (
        <InvoiceModal 
            user={user} 
            activeTable={payingTable} 
            activeOrderId={payingOrderId} 
            onClose={() => { setPayingTable(null); setPayingOrderId(null) }} 
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
            <div>
                <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                    <Layers className="text-emerald-600"/> Sơ đồ bàn
                </h1>
            </div>
            <div className="flex w-full md:w-auto gap-2">
                ...
                {user.role === 'MANAGER' && (
                    <button 
                        onClick={() => { setShowSetupModal(true); setSetupError(''); }} 
                        className="bg-slate-800 text-white px-3 py-2 rounded-xl hover:bg-slate-900 transition shadow-lg flex items-center gap-2 font-bold text-sm whitespace-nowrap"
                    >
            ...
            {uniqueZones.map(z => {
                ...
                return (
                    <button 
                        ...
                        className={`pb-3 px-1 border-b-2 transition-all whitespace-nowrap text-sm font-bold flex items-center gap-2 ${isActive ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
                    >
                        {z} 
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${isActive ? 'bg-emerald-100' : 'bg-slate-100'}`}>
           ...
           <>
             {displayTables.length === 0 ? (
                 <div className="text-center py-20 text-slate-400 flex flex-col items-center">
                     ...
                     <p>{uniqueZones.length === 0 ? "Vui lòng bấm 'Thiết lập quán' để tạo bàn." : `Không tìm thấy bàn ở ${activeZone}`}</p>
                 </div>
             ) : (
                    ...
                    {displayTables.map(table => {
                        ...
                        return (
                            <div 
                                ...
                                className={`relative h-40 rounded-2xl border flex flex-col items-center justify-center gap-2 cursor-pointer transition-all duration-200 group shadow-sm ${isBusy ? 'bg-white border-rose-200 shadow-rose-100' : 'bg-white border-slate-200 hover:border-emerald-500 hover:shadow-md'}`}
                            >
                                ...
                                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-colors mt-2 ${isBusy ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-400 group-hover:bg-emerald-100 group-hover:text-emerald-600'}`}>
                                    <Users size={24}/>
                                </div>
                                ...
                                {isBusy && (
                                    <button 
                                        onClick={(e) => handleQuickPay(e, table)} 
                                        className="absolute bottom-0 left-0 w-full py-2 bg-rose-600 text-white text-sm font-bold rounded-b-xl shadow-sm hover:bg-rose-700 z-10 flex items-center justify-center gap-1"
                                    >
                                        Thanh toán ngay
                                    </button>
                                )}
            ...
            <div className="bg-white rounded-2xl w-full max-w-sm flex flex-col max-h-[90vh] relative z-10 animate-fadeIn shadow-2xl overflow-hidden">
                <div className="p-6 pb-0">
                    ...
                    <h2 className="text-xl font-bold text-slate-800 mb-2 flex items-center gap-2">
                        <Settings className="text-slate-800"/> Thiết lập quán
                    </h2>
                    
                    <div className="bg-orange-50 text-orange-700 p-3 rounded-lg text-xs border border-orange-200 flex gap-2 mb-4">
                        <AlertTriangle size={16} className="shrink-0"/>
                        <span>Lưu ý: Tính năng này sẽ <b>XÓA HẾT</b> các bàn cũ và tạo lại mới.</span>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-2 custom-scrollbar">
                    <form id="setup-form" onSubmit={triggerBulkSetup} className="space-y-6">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 mb-1">Tổng số tầng</label>
                            <input 
                                type="number" min="1" max="20" required 
                                value={setupData.floors} 
                                onChange={e => handleFloorsChange(e.target.value)} 
                                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-center text-lg focus:outline-emerald-500" 
                            />
                        </div>
                        <div className="space-y-3">
                            <label className="block text-xs font-bold text-slate-500 border-b border-slate-100 pb-2">Chi tiết số bàn từng tầng:</label>
                            {Array.from({ length: setupData.floors }).map((_, idx) => { 
                                const floorNum = idx + 1
                                return (
                                    <div key={floorNum} className="flex items-center justify-between gap-4 animate-fadeIn">
                                        <span className="font-bold text-slate-700 text-sm w-20">Tầng {floorNum}</span>
                                        <div className="flex items-center gap-2 flex-1">
                                            <input 
                                                type="number" min="0" max="100" 
                                                value={floorConfigs[floorNum] || 0} 
                                                onChange={(e) => setFloorConfigs({...floorConfigs, [floorNum]: Number(e.target.value)})} 
                                                className="w-full p-2 bg-white border border-slate-200 rounded-lg text-center font-bold text-slate-800 focus:border-emerald-500 focus:outline-none"
                                            />
                                            <span className="text-xs text-slate-400 whitespace-nowrap">bàn</span>
                                        </div>
                                    </div>
                                )
                            })}
                ...
                <div className="p-6 pt-4 border-t border-slate-100 bg-slate-50">
                    <button 
                        form="setup-form" type="submit" disabled={isSettingUp} 
                        className="w-full py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 shadow-lg shadow-emerald-200 flex items-center justify-center gap-2 disabled:opacity-50 transition active:scale-95"
                    >
                        {isSettingUp ? 'Đang khởi tạo...' : 'Xác nhận & Tạo mới'}
                    </button>
                </div>
            </div>
        </div>
