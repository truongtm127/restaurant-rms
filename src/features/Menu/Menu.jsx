...
export default function Menu({ user, activeTable, activeOrderId, setActiveTable, setActiveOrderId, setRoute }) {
             ...
             finalItems = orderItems.map(cartItem => {
               ...
               if (existingItem) {
                 return { 
                   ...
                   qty: cartItem.qty, 
                   note: cartItem.note || '',
                   qtyCompleted: existingItem.qtyCompleted || 0 
                 }
               } else {
  ...
  const addToOrder = async (m) => {
    ...
    try { if (activeTable.status === 'FREE') await updateDoc(doc(db, 'tables', activeTable.id), { status: 'BUSY' }) } catch (e) {}
  }
  ...
  const removeItem = (item) => {
    ...
    if (isLastItem) {
        openConfirm("Hủy đơn", "Hủy đơn và trả bàn?", async () => {
            await deleteDoc(doc(db, 'orders', activeOrderId, 'items', item.id)); await deleteDoc(doc(db, 'orders', activeOrderId));                
            await updateDoc(doc(db, 'tables', activeTable.id), { status: 'FREE' }); setActiveTable(null); setActiveOrderId(null); setRoute('order');
        });
    } else {
        openConfirm("Xóa món", `Xóa "${item.name}"?`, async () => { await deleteDoc(doc(db, 'orders', activeOrderId, 'items', item.id)) });
    }
  }
    ...
    <div className="space-y-4 h-full flex flex-col">
      ...
      {activeTable && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3 rounded-xl border border-slate-200 shadow-sm animate-fadeIn">
          {/* Thông tin bàn & Nút Quay lại */}
          <div className="flex items-center gap-3">
             <button onClick={handleBackToOrder} className="p-2 hover:bg-slate-100 rounded-lg text-slate-500 transition" title="Quay lại"><ArrowLeft size={20}/></button>
             <div className="bg-emerald-100 text-emerald-700 p-2 rounded-lg">🍽️</div>
             <div><div className="text-xs text-slate-500 font-medium uppercase">Đang phục vụ</div><div className="text-sm font-bold text-slate-800">Bàn {activeTable.name || activeTable.id}</div></div>
          </div>
          
          {/* [UPDATED] NÚT THANH TOÁN (Disable nếu giỏ hàng trống) */}
      ...
      {activeOrderId && (
        <div className="bg-white rounded-xl shadow-sm border p-3">
          <div className="flex items-center justify-between mb-2"><div className="font-semibold text-sm">Đơn hàng ({orderItems.length} món)</div><div className="font-bold text-emerald-700">{(cartTotal/1000).toFixed(0)}k</div></div>
          ...
          {/* Ghi chú */}
          <div className="mt-2 pt-2 border-t border-slate-100"><div className="relative"><input value={orderNote} onChange={(e) => setOrderNote(e.target.value)} placeholder="Ghi chú cho bếp..." className="w-full text-xs pl-7 pr-3 py-2 border border-slate-200 rounded-lg bg-slate-50 focus:bg-white focus:border-emerald-500 outline-none"/><FilePenLine size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" /></div></div>
          ...
          <div className="flex justify-end pt-2 mt-2 border-t border-slate-50">
              ...
              >
                <CheckCircle size={16} /> 
                {hasItems ? "Gửi Bếp & Quay lại" : "Quay lại"}
              </button>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 md:flex-row md:items-center">
        <div className="flex-1 relative"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Tìm món..." className="w-full border rounded-lg pl-8 pr-3 py-1.5 text-sm bg-white focus:ring-1 focus:ring-emerald-500 outline-none"/><Search className="w-4 h-4 absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" /></div>
        <div className="flex gap-2">
          <select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="border rounded-lg px-2 py-1.5 bg-white text-sm outline-none cursor-pointer"><option value="popular">Phổ biến</option><option value="newest">Mới nhất</option><option value="priceAsc">Giá tăng</option><option value="priceDesc">Giá giảm</option></select>
          {isManager && <button onClick={openAdd} className="px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-sm hover:bg-emerald-700 flex items-center gap-1 font-medium whitespace-nowrap shadow-sm"><Plus className="w-3.5 h-3.5"/> Thêm</button>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2 pb-4">
        {loading && items.length === 0 ? Array.from({length:16}).map((_,i)=><SkeletonCard key={i}/>) : filtered.length === 0 ? <div className="col-span-full text-center text-sm text-slate-500 py-10">Không tìm thấy món nào.</div> : filtered.map(m => <ItemCard key={m.id} m={m} onEdit={openEdit} onDelete={handleDelete} onAdd={addToOrder} canAdd={!!activeOrderId} canManage={isManager} />)}
      </div>
